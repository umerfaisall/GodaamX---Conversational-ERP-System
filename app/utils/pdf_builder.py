...
def _add_cards_section(pdf: _ReportPDF, cards: dict[str, Any]) -> None:
    ...
    for idx in range(0, len(items), 2):
        for j in range(2):
            ...
            pdf.cell(col_w, 5, _pretty_key(key), new_x="LMARGIN", new_y="NEXT" if j == 1 else "TOP")
            pdf.set_xy(x, pdf.get_y() + 5)
            pdf.set_font(_FONT, "B", 14)
            pdf.set_text_color(*_DARK_TEXT)
            pdf.cell(col_w, 7, _fmt(val), new_x="LMARGIN", new_y="NEXT" if j == 1 else "TOP")
        pdf.ln(14)
    pdf.ln(2)
...
def _add_table(pdf: _ReportPDF, title: str, rows: list[dict[str, Any]]) -> None:
    ...
    for h in headers:
        pdf.cell(col_w, 7, _pretty_key(h), border=1, fill=True, align="C")
    pdf.ln()
    ...
    for row in rows:
        ...
        for h in headers:
            pdf.cell(col_w, 7, _fmt(row.get(h)), border=1, fill=True, align="C")
        pdf.ln()
        fill = not fill
...
def build_dashboard_pdf(dashboard_data: dict[str, Any]) -> bytes:
    ...
    pdf.cell(0, 7, f"Dashboard Role: {role}", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)
    ...
    for key, value in dashboard_data.items():
        ...
        if isinstance(value, list) and value and isinstance(value[0], dict):
            ...
            for k, v in value.items():
                pdf.cell(60, 7, _pretty_key(k), new_x="RIGHT")
                pdf.cell(0, 7, _fmt(v), new_x="LMARGIN", new_y="NEXT")
            pdf.ln(4)
